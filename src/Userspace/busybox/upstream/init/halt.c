...
//config:	default y
//config:	depends on HALT || POWEROFF || REBOOT || SHUTDOWN
//config:	help
...
//config:	default y
//config:	depends on (HALT || POWEROFF || REBOOT || SHUTDOWN) && !INIT
//config:	help
...
//applet:IF_SHUTDOWN(APPLET_ODDNAME(shutdown, halt, BB_DIR_SBIN, BB_SUID_DROP, shutdown))

//kbuild:lib-$(CONFIG_HALT) += halt.o
//kbuild:lib-$(CONFIG_POWEROFF) += halt.o
//kbuild:lib-$(CONFIG_REBOOT) += halt.o
//kbuild:lib-$(CONFIG_SHUTDOWN) += halt.o
...
int halt_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (!ENABLE_HALT && !ENABLE_POWEROFF && ENABLE_REBOOT && !ENABLE_SHUTDOWN)
		which = 2;
	else
	for (which = 0; "hpsr"[which] != applet_name[0]; which++)
		continue;
	if (which == 3)
		which = 2;
	if (applet_name[0] == 's')
...
#ifdef __APPLE__
	flags |= 4; /* no telinit/runlevel path in early PureDarwin userland */
#endif
