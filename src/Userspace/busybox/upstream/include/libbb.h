...
/* optreset lives in libSystem, but this declaration sits inside libbb.h's
 * visibility-push(hidden) region; without an explicit default visibility the
 * compiler assumes it is defined in-image and emits a direct (non-GOT)
 * reference, which ld64 cannot bind to a dylib symbol ("bindingNone"). */
extern int optreset __attribute__((visibility("default")));
#define GETOPT_RESET() (optind = 1, optreset = 1)
